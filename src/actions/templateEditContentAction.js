...
export default async function templateEditContentAction({ request, params }) {
    try {
        const res = await request.json();
        if (res.id) {
            const { id: _, datetime: __, ...rest } = res;
            await uploadDoc(rest, collections.templates, res.id, true);
            return redirect(`/templates/${res.id}`)
        }
